...
export default async function OffersPage() {
  ...
  const offers = result.data.filter((product) => product.oldPrice && product.oldPrice > product.price);
  ...
  return (
    <div className="min-h-screen bg-[#f8f8f7] text-neutral-950">
      ...
      <main className="mx-auto max-w-7xl px-5 py-12 lg:px-8">
        ...
        <p className="mt-3 text-neutral-600">Discounted liquors with a real marked-down price.</p>

        {offers.length === 0 ? (
          <section className="mt-10 rounded-lg bg-white p-16 text-center shadow-sm ring-1 ring-black/5">
            <h2 className="text-2xl font-semibold">No offers right now</h2>
            <p className="mt-2 text-neutral-500">Discounted bottles will appear here when available.</p>
